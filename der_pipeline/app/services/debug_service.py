...
class DebugService:
    ...
    def __init__(self):
        ...
        try:
            self.llm = ChatOpenAI(
                api_key=settings.openai_api_key, model_name="gpt-3.5-turbo", temperature=0.1
            )
    ...
    def analyze_reconciliation_issues(
        self, document_id: int, request: DebugRequest
    ) -> DebugResponse:
        ...
        try:
            formatted_prompt = prompt.format(
                ...
                reconciliation_result=json.dumps(
                    request.input_data.get("reconciliation_result", {}), indent=2
                ),
            ...
            return DebugResponse(
                ...
                ai_analysis=analysis,
                recommendations=analysis.get("recommendations", [])
                + analysis.get("strategy_suggestions", []),
                confidence_score=analysis.get("confidence", 0.8),
            )
        ...
        try:
            formatted_prompt = prompt.format(
                original_extraction=json.dumps(
                    request.input_data.get("original_extraction", {}), indent=2
                ),
    ...
    def _store_debug_session(
        self, document_id: int, request: DebugRequest, analysis: dict[str, Any], confidence: float
    ) -> int:
        ...
        try:
            with Session(get_session().bind) as session:
                stmt = (
                    ...
                    .filter(DebugSession.document_id == document_id)
