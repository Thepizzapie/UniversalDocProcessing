...
from app.schemas import ExtractedRecord, ReconcileDiff
...
class DocumentProcessingCrew:
    ...
    def process_document_complete(
        ...
        try:
            ...
            validation_results = self.validate_data(extraction_results.root, document_text)
            ...
            if external_data:
                reconciliation_results, overall_score = self.reconcile_data(
                    validation_results, external_data, reconcile_strategy
                )
                results["reconciliation_results"] = {
                    "comparisons": reconciliation_results,
                    "overall_score": overall_score,
                    "analysis": self.reconciliation_agent.analyze_discrepancies(
        ...
        if extracted_fields and not isinstance(list(extracted_fields.values())[0], ExtractedField):
            ...
            for key, value in extracted_fields.items():
                if isinstance(value, dict):
                    ...
                    converted_fields[key] = ExtractedField(
                        value=value, confidence=0.5, type_hint="text"
                    )
            extracted_fields = converted_fields

        validated_fields = self.validation_agent.validate_extracted_fields(
            extracted_fields, document_context
        )

        # Also get correction suggestions for low-confidence fields
        suggestions = self.validation_agent.suggest_corrections(validated_fields, document_context)
    ...
    def reconcile_data(
        ...
        return self.reconciliation_agent.reconcile_records(simple_fields, external_data, strategy)
    ...
    def _summarize_validation(self, validated_fields: dict[str, Any]) -> dict[str, Any]:
        ...
        average_confidence = (
            sum(f.confidence for f in validated_fields.values()) / total_fields
            if total_fields > 0
            else 0
        )
        ...
        if results.get("reconciliation_results"):
            ...
            if recon_analysis.get("action_required", False):
                recommendations.append(
                    "Critical discrepancies found - manual reconciliation required"
                )
    ...
    def get_crew_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "process_type": (
                self.crew.process.value
                if hasattr(self.crew.process, "value")
                else str(self.crew.process)
            ),
