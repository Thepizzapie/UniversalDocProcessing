...
from app.schemas import ReconcileDiff, RagSearchRequest
from app.enums import ReconcileStatus, ReconcileStrategy, DocumentType
...
class ReconciliationAgent:
        ...
        def reconcile_data(comparison_data: str) -> str:
            ...
            try:
                prompt = self.reconciliation_prompt.format(
                    ...
                    strategy=comparison_data.split("|||")[2] if len(comparison_data.split("|||")) > 2 else "LOOSE"
                )
    ...
    def reconcile_records(
        ...
        comparison_input = f"{extracted_str}|||{fetched_str}|||{strategy.value}"
    ...
    def _parse_reconciliation_results(
        ...
        try:
            ...
            return self._fallback_reconciliation(extracted_data, fetched_data, ReconcileStrategy.LOOSE)
    ...
    def analyze_discrepancies(
        self,
        reconciliation_results: List[ReconcileDiff]
    ) -> Dict[str, Any]:
        ...
        critical_mismatches = [m for m in mismatches if any(cf in m.field.lower() for cf in critical_fields)]
        ...
        if critical_mismatches:
            analysis["recommendations"].append("Manual review required for critical field discrepancies")
            analysis["recommendations"].append("Verify source data accuracy before proceeding")
        else:
            analysis["recommendations"].append("Minor discrepancies found - consider automated resolution")
            ...
            if search_query:
                ...
                if results:
                    ...
                    for result in results:
                        context_parts.append(f"Reference (similarity: {result.similarity_score:.2f}):")
    ...
    def _build_search_query(self, extracted_data: Dict[str, Any], document_type: DocumentType) -> str:
